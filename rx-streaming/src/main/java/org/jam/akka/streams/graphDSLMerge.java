	...
	public static void main(String[] args) {
		...
		final RunnableGraph<CompletionStage<Done>> graph = RunnableGraph.fromGraph(
				GraphDSL.create(consoleSink, (builder, out) -> {
					...
					Outlet<Integer> outlet = builder.add(source).out();
					...
					Outlet<Integer> outlet_0 = builder.from(bcast.out(0))
							.via(builder.add(f2))
							.out();

					Outlet<Integer> outlet_1 = builder.from(bcast.out(1))
							.via(builder.add(f4))
							.out();

					outlet = builder.from(outlet_1)
							.viaFanIn(merge)
							.out();

					outlet = builder.from(outlet_0)
							.viaFanIn(merge)
							.out();
